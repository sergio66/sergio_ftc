...
% clustcmd -L find_predictors_uplook49_605v3.m 1:2490
%
% otherwise when happy
% clustcmd -q short -n 72 find_predictors_uplook49_605v3.m 1:2490

disp('hello sergio this is "find_predictors_uplook49_605v3.m" ')
JOB
iChan0 = JOB(1);
...
ii=ii+1; vars(ii,:,:) = stemp'*ones(1,100);          names{ii} = 'stemp';     types(ii) = 1;
ii=ii+1; vars(ii,:,:) = ptemp;                       names{ii} = 'ptemp';     types(ii) = 1;
...
ii=ii+1; wgtT = cumsum(pwgt' .* ptemp') ./ cumsum(pwgt');                     
         vars(ii,:,:) = wgtT'/273;                          names{ii} = 'presswgtTx';   types(ii) = 1;
...
ii=ii+1; vars(ii,:,:) = gas_1/1e22;                              names{ii} = 'gas_1';            types(ii) = 2;
ii=ii+1; vars(ii,:,:) = log10(gas_1/1e22);                       names{ii} = 'log(gas_1)';       types(ii) = 2;
ii=ii+1;  vars(ii,:,:) = mm'*ones(1,100);                        names{ii} = 'mm water';         types(ii) = 2;
ii=ii+1;  vars(ii,:,:) = (mm.^2)'*ones(1,100);                   names{ii} = 'mm^2 water';       types(ii) = 2;
ii=ii+1;  vars(ii,:,:) = (mm.^0.5)'*ones(1,100);                 names{ii} = 'sqrt(mm) water';   types(ii) = 2;
ii=ii+1;  vars(ii,:,:) = mm' .* stemp' *ones(1,100);             names{ii} = 'mmwater * stemp '; types(ii) = 2;
ii=ii+1; vars(ii,:,:) = sqrt(gas_1 ./ (ones(49,1)*g1ref));       names{ii} = 'sqrt(g1/g1ref)';   types(ii) = 2;
ii=ii+1; vars(ii,:,:) = (gas_1 ./ (ones(49,1)*g1ref)).^2;        names{ii} = '(g1/g1ref)^2';     types(ii) = 2;
ii=ii+1; wgtW = (pav.*gas_1); vars(ii,:,:) = wgtW/1e21;          names{ii} = 'pressWgtW';        types(ii) = 2;
...
ii=ii+1; wgtW = cumsum(pwgt' .* gas_1') ./ cumsum(pwgt');
         vars(ii,:,:) = wgtW'/1e21;                              names{ii} = 'presswgtWx';       types(ii) = 2;
ii=ii+1; vars(ii,:,:) = squeeze(vars(ii-1,:,:)).^2;              names{ii} = 'presswgtWx.^2';    types(ii) = 2;
ii=ii+1; vars(ii,:,:) = squeeze(vars(iiW,:,:)) .* ... 
                      (ptemp ./ (ones(49,1)*tref));             names{ii} = 'g1/g1ref * T/Tref';       types(ii) = 2;
...
ii=ii+1; vars(ii,:,:) = gas_3/1e19;                             names{ii} = 'gas_3';             types(ii) = 3;
ii=ii+1; vars(ii,:,:) = log10(gas_3/1e29);                      names{ii} = 'log(gas_3)';        types(ii) = 3;
ii=ii+1; vars(ii,:,:) = sqrt(gas_3 ./ (ones(49,1)*g3ref));      names{ii} = 'sqrt(g3/g3ref)';    types(ii) = 3;
ii=ii+1; vars(ii,:,:) = (gas_3 ./ (ones(49,1)*g3ref)).^2;       names{ii} = '(g3/g3ref)^2';      types(ii) = 3;
ii=ii+1; wgtO = (pav.*gas_3); vars(ii,:,:) = wgtO/1e18;         names{ii} = 'pressWgtO';         types(ii) = 3;
ii=ii+1; wgtO = cumsum(pwgt' .* gas_3') ./ cumsum(pwgt');
         vars(ii,:,:) = wgtO'/1e18;                             names{ii} = 'presswgtOx';        types(ii) = 3;
ii=ii+1; vars(ii,:,:) = squeeze(vars(iiO,:,:)) .* ...
                      (ptemp ./ (ones(49,1)*tref));             names{ii} = 'g3/g3ref * T/Tref'; types(ii) = 3;
...
figure(6);
ix = [2 5 7]; plot(1:100,xCW(:,ix),1:100,waterOD(ix,:),'r')