...
%F   = [F   zeros(length(w),1)];
%FW  = [FW  zeros(length(w),1)];
%FWO = [FWO zeros(length(w),1)];
...
%hold on
%plot(Feff,1:nn,'bo-',Weff,1:nn,'g+-',Oeff,1:nn,'rs-'); 
%hold off
...
for ix = length(ptemp) : -1 : 1
  ...
  tixm1 = Feff(:,ix-0) .* Oeff(:,ix-0) .* Weff(:,ix-0);
...
figure(3)
plot(fc,rad2bt(fc,qcx),fc,rad2bt(fc,rxconv),'r')
