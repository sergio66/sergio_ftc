...
[h,ha,p,pa] = rtpread('regr_usstd_11offsets.op.rtp');
for ip = 1 : 11

  [hN,pN] = subset_rtp(h,p,[],[],ip);

  fnameIN = ['/asl/s1/sergio/BBAERI/WV/prof_11us_std_' num2str(ip) '_B_' num2str(B) '.dat'];
  [d1,w] = readkcstd(fnameIN);

  fnameIN = ['/asl/s1/sergio/BBAERI/CO2/prof_11us_std_' num2str(ip) '_B_' num2str(B) '.dat'];
  [d2,w] = readkcstd(fnameIN);

  fnameIN = ['/asl/s1/sergio/BBAERI/O3/prof_11us_std_' num2str(ip) '_B_' num2str(B) '.dat'];
  [d3,w] = readkcstd(fnameIN);

  fnameIN = ['/asl/s1/sergio/BBAERI/OTHERS/prof_11us_std_' num2str(ip) '_B_' num2str(B) '.dat'];
  ...
  fout = ['CONV_trans/prof_11us_std_' num2str(ip) '_band_' num2str(B) '.mat'];
