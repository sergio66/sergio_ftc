...
for ii = 1 : 2390
  ...
  ystr = ['COEFFS/605/water605_chan' num2str(ii) '.mat'];
  loader = ['y = load(''' ystr ''');'];
  eval(loader);
  CW(ii,:,:) = y.xCW;

  ystr = ['COEFFS/605/fixed605_chan' num2str(ii) '.mat'];
  loader = ['y = load(''' ystr ''');'];
  eval(loader);
  CF(ii,:,:) = y.xCF;

  ystr = ['COEFFS/605/ozone605_chan' num2str(ii) '.mat'];
  loader = ['y = load(''' ystr ''');'];
  eval(loader);
  COz(ii,:,:) = y.xCOz;
end
disp('loaded in 2390 F W O files ...')
