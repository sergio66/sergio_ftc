...
boo1  = predsets.W.set1_1;  boo1  = boo1(layers,profile_list,:);  boo1 = boo1(:);
boo2  = predsets.W.set1_2;  boo2  = boo2(layers,profile_list,:);  boo2 = boo2(:);
boo3  = predsets.W.set1_3;  boo3  = boo3(layers,profile_list,:);  boo3 = boo3(:);
boo4  = predsets.W.set1_4;  boo4  = boo4(layers,profile_list,:);  boo4 = boo4(:);
boo5  = predsets.W.set1_5;  boo5  = boo5(layers,profile_list,:);  boo5 = boo5(:);
boo6  = predsets.W.set1_6;  boo6  = boo6(layers,profile_list,:);  boo6 = boo6(:);
boo7  = predsets.W.set1_7;  boo7  = boo7(layers,profile_list,:);  boo7 = boo7(:);
boo8  = predsets.W.set1_8;  boo8  = boo8(layers,profile_list,:);  boo8 = boo8(:);
boo9  = predsets.W.set1_9;  boo9  = boo9(layers,profile_list,:);  boo9 = boo9(:);
boo10 = predsets.W.set1_10; boo10 = boo10(layers,profile_list,:); boo10 = boo10(:);
boo11 = predsets.W.set1_11; boo11 = boo11(layers,profile_list,:); boo11 = boo11(:);
booU  = ones(size(boo1));
...
matr11 = [booU boo1 boo2 boo3 boo4 boo5 boo6 boo7 boo8 boo9 boo10 boo11];
matr8 = [booU boo1 boo2 boo3 boo4 boo5 boo6 boo7 boo8];

matr = matr11;
matr = matr8;  %% best for Ch 1614 ???
...
oo = find(xOD < hingeODs(1));    wgtOD(oo) = 1;
oo = find(xOD > hingeODs(end));  wgtOD(oo) = 1;
wgtmatr = diag(wgtOD);
...
figure(1); plot(xOD,predict,'.',xOD,xOD,'r.'); xlabel(strx); ylabel(stry)
figure(2); plot(xOD,therror,'.'); xlabel(strx); ylabel([strx '-' stry])
figure(3); scatter(xOD,therror,10,lpaP,'filled'); colorbar; colormap jet
figure(3); pcerror = (xOD-predict)./xOD*100; scatter(xOD,pcerror,10,lpaP,'filled'); colorbar; colormap jet
figure(3); plot(lpaP,pcerror,'.'); xlabel('profile'); ylabel('percent error')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
matr = [boo2 boo7 booS booT booW booU];
fitc = matr\xOD;
