...
if regr49
   ...
   g = load('optran_chan1099_v1_regr49_jan23test');
...
koall = nan(300,nprof);
for i=1:300
   if size(g.netfinal(i).net) > 0
%      kcall(i,:) = netfinal(i).calc;
      koall(i,:) = g.netfinal(i).obs;
   end      
...
keyboard
%Now, need to compute calcs for 49 profs using net for 704

k1 = ~isnan(g.net_prof_optran_index);
k2 = ~isnan(koall);
kx = and(k1,k2);

for i=1:nprof
   tk(:,i) = interp1(g.net_prof_optran_index(kx(:,i),i),koall(kx(:,i),i),1:100,'linear');
