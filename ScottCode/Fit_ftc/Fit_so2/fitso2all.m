...
% Clip tauzs>tauz
iclip = find(tauzs > tauz);
tauzs(iclip)=tauz(iclip);

% Clip transmittance>1
iclip = find(tauzs > 1.0);
tauzs(iclip) = 1.0;
tauz(iclip) = 1.0;
iclip = find(tauz > 1.0);
tauz(iclip) = 1.0;
...
tauz(iclip) = tauclip;
tauzs(iclip) = tauclip;
iclip = find(tauzs < tauclip);
tauzs(iclip) = tauclip;


% Calc offset SO2 layer optical depth
% Note: tauzs = tauz except for offset SO2 in layer L only
odso2=-log( tauzs ) + log( tauz );
clear tauz
zodso2=-log( tauzs );
clear tauzs
...
%      ind=find(lk > 1.5E-5 & lkz < 6.8);  % used this with IASI
      npts=length(ind);
...
outname = [csname '_so2_' band];
eval(['save ' outname ' ichan fchan maxso2 allcoef so2_mult'])
