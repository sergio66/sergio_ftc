...
% Update: 01 Oct 2008, S.Hannon - add check of secang
% Update: 27 Apr 2009, S.Hannon - minor update for CrIS
% Update: 10 Nov 2009, S.Hannon - minor update for CrIS "g4"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Number of CrIS channels
nchan = 1329;
...
% Check secang from the two files agree
junk = max(abs(secang - rdown_secang));
if (junk > 0.0001)
   % See if re-ordering rdown helps
   nangprof = nang.*nprof;
   ind = zeros(1,nangprof);
   jj=1:6;
   ioffset=(jj - 1)*nprof;
   for ii=1:nprof
      ind(jj) = ii + ioffset;
   ...
   clear ii jj ioffset
   rdown_secang = rdown_secang(ind);
   junk = max(abs(secang - rdown_secang));
   if (junk > 0.0001)
      disp('Error: rdown and thermdata secangs do not match')
   else
      disp('WARNING! re-ordered rdown so secang data matchs thermdata')
      disp('Carefully review output to make sure things look OK')
      rdown = rdown(:,ind);
      pause(5)
   end
end
