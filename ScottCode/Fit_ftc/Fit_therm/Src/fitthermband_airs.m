% Program xfitthermband
%
% Reads in reflected thermal regression data and fits for the
% F-factor coefficients.  This "x" version for fit using "rdown"
% term calculationed using the fast model.
%

% Created: 05 April 2005, Scott Hannon - based on xfitthermchan.m and
%    fitthermband.m
% Update: 22 Apr 2008, S.Hannon - created for April 2008 AIRS production
%    based on xfitthermband2.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% version 2 expects input rdown is NOT multiplied by tauz
...
dir ../Data
inname = input('Enter name of rdown data : ','s');
load(inname);
rdown = rdown(ichan,:)/1000;  % undo wrtrtp.f multiply by 1000


% Filename prefix (ie name minus band) for input refl therm data file name
inname = input('Enter name of therm data : ','s');
load(inname);


% Name for output F-factor coefficient file name
outname = input('Enter name of output file to create : ','s');
...
%%%
      ...
      if (npts > nprofmin+6 & nsec > 1)
         ...
         B = [B, term2b']; %'
         %
         if (npts > nprofmin+12 & maxtau > 1E-2)
            nterm = 3;
            A = [A, term3'];  %'
            B = [B, term3b']; %'
            %
            if (npts > nprofmin+18 & maxtau > 1E-1)
               nterm = 4;
               A = [A, term4'];  %'
               B = [B, term4b']; %'
               %
...
eval(['save ' outname ' coefall rmsall rmsall2 freq idchan band ' ...
   ' dbt250 fall tauz'])
