...
name=input('Enter name of cris data {eg cris, crishiresh4} : ','s');
...
% Clip tauz12>tauz
iclip = find(tauz12 > tauz);
%tauz9(iclip)=tauz(iclip); % wrong!
tauz12(iclip)=tauz(iclip); % bug fix added 18 Apr 2008

% Clip transmittance>1
iclip = find(tauz12 > 1.0);
tauz12(iclip) = 1.0;
tauz(iclip) = 1.0;
iclip = find(tauz > 1.0);
tauz(iclip) = 1.0;
...
outname=[name '_allcoef_hno3']
eval(['save ' outname ' ichan fchan maxhno3 allcoef hno3_mult rmsperr lkmean'])
