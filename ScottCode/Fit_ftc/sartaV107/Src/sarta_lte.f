...
C      --------------
     ...
     $    SALT, SUNANG, PSURF, TSURF, CO2PPM, XEMIS, FEMIS, XRHO, FRHO,
...
C      Reset FAMNT if input RTP lacks CO2 profile
       IF (ICO2 .LT. 1) THEN
          DO L=1,LBOT
             FAMNT(L)=RFAMNT(L)
          ENDDO
       ENDIF
C      Reset SAMNT if input RTP lacks SO2 profile
       IF (ISO2 .LT. 1) THEN
          DO L=1,LBOT
             SAMNT(L)=RSAMNT(L)
          ENDDO
       ENDIF
C      Reset HAMNT if input RTP lacks HNO3 profile
       IF (IHNO3 .LT. 1) THEN
          DO L=1,LBOT
             HAMNT(L)=RHAMNT(L)
          ENDDO
       ENDIF
C      Reset NAMNT if input RTP lacks N2O profile
       IF (IN2O .LT. 1) THEN
          DO L=1,LBOT
             NAMNT(L)=RNAMNT(L)
          ENDDO
...
C
     ...
     $    RPRES,SECANG,   LAT,    FX,   RDZ,FIXMUL,CONPRD,
...
C      Reset CO2MLT if input RTP lacks CO2 profile
       IF (ICO2 .LT. 1) THEN
          CO2MLT(1)=100.0*(CO2PPM - CO2STD)/(3.0*CO2STD)
          DO L=2,MAXLAY
             CO2MLT(L)=CO2MLT(1)
          ENDDO
       ENDIF
...
C      ---------------------------------------------------
       CALL SETEMS( NCHAN, NEMIS, NRHO, FREQ, XEMIS, FEMIS,
     $    XRHO, FRHO, LRHOT, EMIS, RHOSUN, RHOTHR)
C
...
ccc this block commented out to force lte
c       IF (DOSUN) THEN
c          CALL CALNTE ( INDCHN, TEMP, SUNCOS, SCOS1, SECANG(1),
c     $       NCHNTE, CLISTN, COEFN, RAD )
c       ENDIF
ccc
