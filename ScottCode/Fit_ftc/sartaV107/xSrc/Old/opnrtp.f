...
c       INTEGER     IG
       ...
       INTEGER   MRHO        ! max number of rho pts
...
C      -------------------------
 ...
 1004     FORMAT('Error! input RTP has no emissivity info')
          STOP
       ENDIF
C      Note: no need to check MRHO; if no RHO data will use (1-emis)/pi
ccc
...
C
 ...
 1008     FORMAT('ERROR! input RTP HEAD.memis=',I4,
     $    ' exceeds MXEMIS=',I4)
       ENDIF
       IF (MRHO .GT. MXEMIS) THEN
          WRITE(IOERR,1009) MRHO, MXEMIS
 1009     FORMAT('ERROR! input RTP HEAD.mrho=',I4,
     $    ' exceeds MXEMIS=',I4)
       ENDIF
C
       NUMBER=HEAD.pfields
...
C
     ...
     $    // '; cloud=' // CJUNK3(1:K) // '; LRHOT=' // CRHOT // CHAR(0)
       J=LENNB(COMMNT)
