...
C    31 Aug 99 Scott Hannon      Add LSTGAS & IGAS[FWO] & IDFIX to
...
C
 ...
 1080     FORMAT('Error, unexpected breakout order for Water.',/,
     $    'Expecting water as gas #2 or 3, but "farray.f" LSTGAS has'
     $    ' it as #',I2)
           STOP
       ENDIF
C
 ...
 1070     FORMAT('Error, unexpected number of O predictors.',/,
     $    'Expecting 5, but "farray.f" has NPREDO = ',I2'.')
          STOP
       ENDIF
       IF ((IGASO .NE. 3) .AND. (IGASO .NE. 2)) THEN
          WRITE(IOERR,1060) IGASO
 1060     FORMAT('Error, unexpected breakout order for Ozone.',/,
     $    'Expecting ozone as gas #3 or 2, but "farray.f" LSTGAS has'
     $    ' it as #',I2)
           STOP
       ENDIF
C
...
C      -------------------------------------------
C      Note: for FWO, AMTREF and AMTPRO are ordered F,W,O
C
...
C               Water (NPREDW = 11)
                ...
                PRED(IOFFST(IGASW)+ 8,IPRO,IANG,ILAY)=RJUNK*ZJUNK
                PRED(IOFFST(IGASW)+ 9,IPRO,IANG,ILAY)=AJUNK*AJUNK*AJUNK
                PRED(IOFFST(IGASW)+10,IPRO,IANG,ILAY)=A_W
                PRED(IOFFST(IGASW)+11,IPRO,IANG,ILAY)=AJUNK*DT*ABS(DT)
C
C
C               Ozone (NPREDO = 5)
