...
C    31 Aug 99 Scott Hannon      Add LSTGAS & SECANG; change wght3 to
C                                   WGHT2A; change NPREDW from 13 to 11
C                                   and delete "k big" stuff.
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fitw_fwo expected NUMGAS=3, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR.
 ...
 1020     FORMAT('Error: fitw_fwo expected LSTGAS=(',I2,
     $    ', [1 or 3], [3 or 1]), not (',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDW .NE. 11) THEN
          WRITE(IOERR,1030) NPREDW
 1030     FORMAT('Error: fitw_fwo expecting NPREDW=11, not ',I2)
          STOP
       ENDIF
C
C
