...
!CALL PROTOCOL:
C    FITW(LSTGAS, ICHAN, NDTPTS, NNEG, IOFFST, PRED, KLTOS, KLAYER,
...
C    12 Jun 97 Scott Hannon      Modified for sun
C    30 Aug 99 Scott Hannon      Add LSTGAS stuff
...
C      =================================================================
       SUBROUTINE FITW(LSTGAS, ICHAN, NDTPTS, NNEG, IOFFST, PRED,
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fitw_fcow expected NUMGAS=4, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR.
 ...
 1020     FORMAT('Error: fitw_fcow expected LSTGAS=(',I2,
     $    ',[5 or 3], [3 or 5], 1), not (',I2,',',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDW .NE. 13) THEN
          WRITE(IOERR,1030) NPREDW
 1030     FORMAT('Error: fitw_fcow expecting NPREDW=13, not ',I2)
          STOP
       ENDIF
C
C
...
C
          ...
          RATNEG=FLOAT( NNEG(ILAY,IGASW) )/
     $       FLOAT( MPTS + NNEG(ILAY,IGASW) )
          XKW=KMAX(ILAY,IGASW)
          XFCO=KMIN(ILAY,IGASF) + KMIN(ILAY,IGASC) + KMIN(ILAY,IGASO)
          XCZ=KZMAX(ILAY,IGASC)
          XKZ=KZMIN(ILAY,IGASF) + KZMIN(ILAY,IGASC) + KZMIN(ILAY,IGASO)
     $       + KZMIN(ILAY,IGASW)
          RATK=XKW/XFCO
C
C         ---------------------------------
...
C         ---------------------------------
C
          IF ((MPTS .GT. MINPTS(1)) .AND. (RATNEG .LT. 0.50)) THEN
             MXPRED=1
             LUSE(1)=.TRUE.
             MXUSE=1
C
             IF ((MPTS .GT. MINPTS(2)) .AND. (XKW .GT. 1.0E-4) .AND.
     $       (XKZ .LT. 10.0) .AND. (RATNEG .LT. 0.25)) THEN
                MXPRED=2
                LUSE(2)=.TRUE.
                MXUSE=MXUSE+1
C
...
C
                   IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $             (XKZ .LT. 8.0) .AND. (RATNEG .LT. 0.10)) THEN
                      MXPRED=4
                      LUSE(4)=.TRUE.
                      MXUSE=MXUSE + 1
C
                      IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                (XKW .GT. 2.0E-4) .AND. (RATNEG .LT. 0.08)) THEN
                         MXPRED=5
                         LUSE(5)=.TRUE.
                         MXUSE=MXUSE + 1
C
                         IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                   (XKW .GT. 4.0E-4) .AND. (XKZ .LT. 7.0) .AND.
     $                   (RATNEG .LT. 0.06) .AND. (RATK .GT. 0.02))
     $                   THEN
                            MXPRED=6
                            LUSE(6)=.TRUE.
                            MXUSE=MXUSE + 1
C
                            IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                      (XKW .GT. 7.0E-4) .AND. (RATNEG .LT. 0.05)
     $                      .AND. (RATK .GT. 0.03)) THEN
                               MXPRED=7
                               LUSE(7)=.TRUE.
                               MXUSE=MXUSE + 1
C
                               IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                         (XKW .GT. 1.0E-3) .AND. (XKZ .LT. 6.0)
     $                         .AND. (RATNEG .LT. 0.04) .AND.
     $                         (RATK .GT. 0.04)) THEN
                                  IF (ILAY .GT. 2) THEN
                                     MXPRED=8
                                     LUSE(8)=.TRUE.
