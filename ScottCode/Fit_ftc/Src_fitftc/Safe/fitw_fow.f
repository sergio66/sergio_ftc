...
C    30 Aug 99 Scott Hannon      Add LSTGAS & SECANG; change wght3 to
C                                   WGHT2A
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fitw_fow expected NUMGAS=3, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR. (IGASO .NE. 2) .OR.
     $    (IGASW .NE. 3)) THEN
          WRITE(IOERR,1020) IDFIX, (LSTGAS(I),I=1,3)
 1020     FORMAT('Error: fitw_fow expected LSTGAS=(',I2,
     $    ', 3, 1), not (',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDW .NE. 11) THEN
          WRITE(IOERR,1030) NPREDW
 1030     FORMAT('Error: fitw_fow expecting NPREDW=11, not ',I2)
          STOP
       ENDIF
C
C
...
C
          ...
          XKW=KMAX(ILAY,IGASW)
          XFO=KMIN(ILAY,IGASF) + KMIN(ILAY,IGASO)
          XOZ=KZMAX(ILAY,IGASO)
          XKZ=KZMIN(ILAY,IGASF) + KZMIN(ILAY,IGASO) + KZMIN(ILAY,IGASW)
          RATK=XKW/XFO
C
C         ---------------------------------
...
C
     ...
     $             (XKW .GT. 2.0E-4)) THEN
                      IF ((RATK .GT. 5.0E-2) .AND. (XKW .GT. 1.0E-3)
     $                .AND. (ILAY .GT. 3) .AND. (XOZ .GT. 1.0E-1))
     $                THEN
                         MXPRED=4
                         LUSE(4)=.TRUE.
                         MXUSE=MXUSE + 1
...
C
     ...
     $                               .AND. (RATK .GT. 1.0E-1) .AND.
     $                               (ILAY .GT. 3) .AND.
     $                               (XKW .GT. 1.0E-2)) THEN
                                        IF (XOZ .GT. 2.0E-1) THEN
                                           MXPRED=10
                                           LUSE(10)=.TRUE.
                                           MXUSE=MXUSE + 1
                                        ENDIF
