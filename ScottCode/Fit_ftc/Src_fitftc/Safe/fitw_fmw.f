...
C    INTEGER    ICHAN          Profile no.        none
...
C    30 Aug 99 Scott Hannon      Add LSTGAS and SECANG; change NPREDW
C                                   to 11 by last two "k big" terms;
C                                   change to WGHT2A from wght3
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fitw_fmw expected NUMGAS=3, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR. (IGASM .NE. 2) .OR.
     $    (IGASW .NE. 3)) THEN
          WRITE(IOERR,1020) IDFIX, (LSTGAS(I),I=1,3)
 1020     FORMAT('Error: fitw_fmw expected LSTGAS=(',I2,
     $    ', 6, 1), not (',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDW .NE. 11) THEN
          WRITE(IOERR,1030) NPREDW
 1030     FORMAT('Error: fitw_fmw expecting NPREDW=11, not ',I2)
          STOP
       ENDIF
C
C
...
C
          ...
          XKW=KMAX(ILAY,IGASW)
          XFM=KMIN(ILAY,IGASF) + KMIN(ILAY,IGASM)
          XMZ=KZMAX(ILAY,IGASM)
          XKZ=KZMIN(ILAY,IGASF) + KZMIN(ILAY,IGASM) + KZMIN(ILAY,IGASW)
          RATK=XKW/XFM
C
C         ---------------------------------
...
C
     ...
     $    (NEGPTS .LT. 20)) THEN
             ...
             MXUSE=1
C
             IF ((MPTS .GT. MINPTS(2)) .AND. (XKW .GT. 1.0E-5) .AND.
...
C
     ...
     $          .AND. (NEGPTS .LT. 5)) THEN
                   IF ((RATK .GT. 5.0E-2) .AND. (XKW .GT. 2.0E-3)
     $             .AND. (ILAY .GT. 3)) THEN
                      MXPRED=3
                      LUSE(3)=.TRUE.
                      MXUSE=3
...
C
                      IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                (XKW .GT. 4.0E-4) .AND. (XKZ .LT. 7.0E+0)) THEN
                         MXPRED=5
                         LUSE(5)=.TRUE.
                         MXUSE=MXUSE + 1
C
                         IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                   (XKW .GT. 7.0E-4) .AND. (RATK .GT. 5.0E-2))
     $                   THEN
                            MXPRED=6
                            LUSE(6)=.TRUE.
                            MXUSE=MXUSE + 1
C
                            IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                      (XKW .GT. 1.0E-3) .AND. (XKZ .LT. 6.0E+0))
     $                      THEN
                               MXPRED=7
                               LUSE(7)=.TRUE.
                               MXUSE=MXUSE + 1
