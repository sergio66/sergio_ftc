c This version for "mw" (medium water) 7 term water fit
C=======================================================================
C=======================================================================
C
...
C    FOW, it is not recommended for cases where the ozone obsorption
C    is significant.
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fitw_fwo_mw7 expected NUMGAS=3, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR.
 ...
 1020     FORMAT('Error: fitw_fwo_mw7 expected LSTGAS=(',I2,
     $    ',[1 or 3], [3 or 1]), not (',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDW .NE. 7) THEN
          WRITE(IOERR,1030) NPREDW
 1030     FORMAT('Error: fitw_fwo_mw7 expecting NPREDW=7, not ',I2)
          STOP
       ENDIF
C
C
...
C
     ...
     $       (XKZ .LT. 10.0) .AND. (RATNEG .LT. 0.20) ) then
c             .AND. (RATK .GT. 0.005)) THEN
                MXPRED=2
                LUSE(2)=.TRUE.
                MXUSE=2
C
                IF ((MPTS .GT. MINPTS(3)) .AND. (XKW .GT. 1.0E-4)
     $          .AND. (XKZ .LT. 8.0) .AND. (RATNEG .LT. 0.10) ) then
c                .AND. (RATK .GT. 0.01)) THEN
                   MXPRED=3
                   LUSE(3)=.TRUE.
                   MXUSE=3
C
