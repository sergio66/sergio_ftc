c This version is for "bf" (big fixed) 11 term fixed gases fit
C=======================================================================
C=======================================================================
C
...
!CALL PROTOCOL:
C    FITF(LSTGAS, ICHAN,NUANG,NDTPTS, NNEG, IOFFST, PRED, KLTOS, KLAYER,
...
C    This version is for FWO and FOW.  Although it works for FOW, it
...
C    --------- ----------------- ---------------------------------------
C    25 Jun 97 Scott Hannon      Created from fitf for FWO
C    27 Aug 99 Scott Hannon      Add LSTGAS stuff
...
C      =================================================================
       SUBROUTINE FITF(LSTGAS, ICHAN,NUANG,NDTPTS, NNEG, IOFFST, PRED,
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fitf_fwo_bf11 expected NUMGAS=3, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR.
 ...
 1020     FORMAT('Error: fitf_fwo_bf11 expected LSTGAS=(',I2,
     $    ',[1 or 3], [3 or 1]), not (',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDF .NE. 11) THEN
          WRITE(IOERR,1030) NPREDF
 1030     FORMAT('Error: fitf_fwo_bf11 expecting NPREDF=11, not ',I2)
          STOP
       ENDIF
C
C
...
C      ------------------------------------------
       DO ILAY=1,MAXLAY
C
          IUANG=NUANG(ILAY,IGASF)
          MPTS=NDTPTS(ILAY,IGASF)
          XKF=KMAX(ILAY,IGASF)
          XKFWO=KMIN(ILAY,IGASF) + KMIN(ILAY,IGASW) + KMIN(ILAY,IGASO)
...
C
     ...
     $       .AND. (XKZ .LT. 10.0) .AND. (RATK .GT. 5.0E-3)) THEN
                MXPRED=4
                LUSE(4)=.TRUE.
                MXUSE=MXUSE+1
C
...
C
                         IF ((MPTS .GT. MINPTS(MXUSE+1)) .AND.
     $                   (RATK .GT. 0.03) .AND. (IUANG .GE. 3)) THEN
                            IF (ILAY .GT. 2) THEN
                               MXPRED=8
                               LUSE(8)=.TRUE.
                               MXUSE=MXUSE+1
