...
C    30 Aug 99 Scott Hannon      Add LSTGAS stuff; change from wght1 to
C                                   WGHT2A
...
C      Check the gases are as expected
 ...
 1010     FORMAT('Error: fito_fcow expected NUMGAS=4, not ',I2)
          STOP
       ENDIF
       IF ((IGASF .NE. 1) .OR.
 ...
 1020     FORMAT('Error: fito_fcow expected LSTGAS=(',I2,
     $    ',[5 or 3], [3 or 5], 1), not (',I2,',',I2,',',I2,',',I2,')')
          STOP
       ENDIF
       IF (NPREDO .NE. 3) THEN
          WRITE(IOERR,1030) NPREDO
 1030     FORMAT('Error: fito_fcow expecting NPREDO=3, not ',I2)
          STOP
       ENDIF
C
...
C
          ...
          XKO=KMAX(ILAY,IGASO)
          XFC=KMIN(ILAY,IGASF) + KMIN(ILAY,IGASC)
...
C
     ...
     $       (XKZ .LT. 1.0E+1) .AND. (RATK .GT. 1.0E-2)) THEN
                MXPRED=2
                LUSE(2)=.TRUE.
                MXUSE=2
C
                IF ((MPTS .GT. MINPTS(3)) .AND. (NEGPTS .LT. 10) .AND.
     $          (XKZ .LT. 7.0E+0) .AND. (RATK .GT. 5.0E-2)) THEN
                   MXPRED=3
                   LUSE(3)=.TRUE.
                   MXUSE=3
                ENDIF
