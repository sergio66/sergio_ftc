c This version for "mfbw" (medium fixed & big water)
C=======================================================================
C=======================================================================
C
...
C    REAL ARR   AMTREF         Ref prof amounts   kmol/cm^2
C    REAL ARR   TPRO           Profile temp       K
C    REAL ARR   AMTPRO         Profile amounts    kmol/cm^2
...
C    --------- ----------------- ---------------------------------------
C     7 Jul 97 Scott Hannon      Created
C    26 Aug 99 Scott Hannon      Add LSTGAS & IGAS[FWO] & IDFIX to
...
C      for F,W,O are what is expected.
C      -----------------------------------
       IF (NPREDF .NE. 8) THEN
...
C               Fixed (NPREDF = 8)
     ...
     $             SECANG(IANG)
                ...
                PRED(IOFFST(IGASF)+8,IPRO,IANG,ILAY)=sqrt(SECANG(IANG))
C
C               Water (NPREDW = 13)
                ...
                PRED(IOFFST(IGASW)+ 4,IPRO,IANG,ILAY)=AJUNK*AJUNK
                PRED(IOFFST(IGASW)+ 5,IPRO,IANG,ILAY)=AJUNK*RJUNK*DT
                PRED(IOFFST(IGASW)+ 6,IPRO,IANG,ILAY)=AJUNK*AJUNK*AJUNK
                PRED(IOFFST(IGASW)+ 7,IPRO,IANG,ILAY)=AJUNK*SECANG(IANG)
                PRED(IOFFST(IGASW)+ 8,IPRO,IANG,ILAY)=AJUNK*ZJUNK
                PRED(IOFFST(IGASW)+ 9,IPRO,IANG,ILAY)=AJUNK*RJUNK*ZJUNK
                PRED(IOFFST(IGASW)+10,IPRO,IANG,ILAY)=AJUNK*SQRT(RJUNK)
                PRED(IOFFST(IGASW)+11,IPRO,IANG,ILAY)=AJUNK*AJUNK*ZJUNK
                PRED(IOFFST(IGASW)+12,IPRO,IANG,ILAY)=AJUNK*A_W
                PRED(IOFFST(IGASW)+13,IPRO,IANG,ILAY)=AJUNK*RJUNK*
     $             SQRT(RJUNK)
C
C               Ozone (NPREDO = 1)
