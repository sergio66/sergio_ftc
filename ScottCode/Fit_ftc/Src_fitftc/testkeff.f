...
C
ccc
       if (IPRO .EQ. 6 .AND. ILAY .EQ. 100 .AND. IGAS .EQ. 3) then
       write(6,*) 'about to check DK=',DK
       endif
...
ccc
       if (IPRO .EQ. 6 .AND. ILAY .EQ. 100 .AND. IGAS .EQ. 3) then
       write(6,*) 'passed DK=',DK
       IF ((DK .GT. DKMIN) .AND. (DK .LT. DKMAX)) THEN
       write(6,*) 'recheck passed DK, DKZ, DKZLA, XK',
     $    DK, DKZ(IGAS), DKZLA(IGAS), XK
       ELSE
       write(6,*) 'recheck FAILED DK, DKZ, DKZLA, XK',
     $    DK, DKZ(IGAS), DKZLA(IGAS), XK
       ENDIF
       endif
ccc
