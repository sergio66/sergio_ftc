...
C
                ...
                IF (DTXZ .GT. 1.0D-18 .AND. DTXZ .LE. 1.0D+0) THEN
                   DKXZ=-DLOG( DTXZ )
C
                   DTXWZ=DBLE( TAUXW(K,L,J) )
                   IF (DTXWZ .GT. 1.0D-18 .AND. DTXWZ .LE. 1.0D+0) THEN
                      DKWZ=-DLOG( DTXWZ/DTXZ )
                      WODZ(L,IJ)=SNGL( DKWZ )
C
                      DELTAK=DKWZ - DKWZM1
                      IF (DKWZM1 .GE. 0.0D+0 .AND. DELTAK .GT. 1.0D-6)
     $                THEN
                         WK(L,IJ)=SNGL( DELTAK )/
     $                      ( PROANG(K,J)*WAP(K,L) )
                      ELSE
