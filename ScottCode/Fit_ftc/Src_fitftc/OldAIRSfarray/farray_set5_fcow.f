C***********************************************************************
C This version for set5 "bfsw" using FCOWp data files
...
C
       ...
       PARAMETER(DKMIN=5.0D-7)
       PARAMETER(DKMAX=1.0D+2)
C
...
C      Note: these values are specific to FCOWp conv trans data files
       ...
       PARAMETER(NUMSET=5)  ! data files are FCOWp = F,FC,FCO,FCOW,FCOWp
C
C      Note: currently set for MAXBOG=6
       ...
       PARAMETER( IBOK1=2 )  ! FC=index 2 (treat FC as F)
       PARAMETER( IBOK2=3 )  ! FCO=index 3
       PARAMETER( IBOK3=4 )  ! FCOW=index 4
       PARAMETER( IBOK4=0 )
       PARAMETER( IBOK5=0 )
       PARAMETER( IBOK6=0 )
C
