...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Update: 24 Apr 2008, Scott Hannon - minor update for April 2008 production

load refprof_n2o.mat

csname = input('Enter channel set name (m130x, m150, etc) : ','s');
band = input('Enter band name (long, short) : ','s');

datafile=input('Enter name of N2O data file: ','s');
...
% Clip tauz>tauzx  (tauzx has 25% *less* N2O than tauz)
iclip = find(tauz > tauzx);
tauzx(iclip)=tauz(iclip);
disp(['number of data points with tauz > tauzx : ' int2str(length(iclip))])
...
%      ind = find(lk > 2E-5 & lkz < 6.7); % for IASI
      npts = length(ind);
