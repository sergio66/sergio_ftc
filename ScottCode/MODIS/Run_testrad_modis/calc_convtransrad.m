...
if (strcmp(typeband,'fow_long'))
   load ../MODIS_ctransradfow_long/ctransrad_fow_long_1.mat
end
if (strcmp(typeband,'fmw'))
   load ../MODIS_ctransradfow_long/ctransrad_fow_long_1.mat
   fmean = fmean(7:10,:);
end
if (strcmp(typeband,'fow_short'))
   load ../MODIS_ctransradfow_short/ctransrad_fow_short_1.mat
end
if (strcmp(typeband,'fcow'))
   load ../MODIS_ctransradfow_short/ctransrad_fow_short_1.mat
   fmean = fmean(1:2,:);
end
...
for ii = 1:nprof;
   ...
   fmeanx = zeros(nchan, nlay+1);
   temp = [prof.ptemp(1:nlay,ii); reg_all_tsurf(ii)];
   x = (temp - tmin)/100;
   for jj = 1:nchan
      kk = find( clist == ichan(jj) );
      pcoef = pcoefall(kk,:);
%      fmeanx(jj,:) = fnom(kk) + polyval(pcoef,x);
      fmeanx(jj,:) = fmean(jj) + polyval(pcoef,x);
   end
   %%%
   ...
   planck = bt2rad( fmeanx(:,nlay+1), reg_all_tsurf(ii) ) * ones(1, nang);
...
nang = nangrad;
secang = secangrad;
eval(['save newradall_' typeband ' radall radtransall fchan ichan typeband nang nprof nchan bttransall btall temp_all h2o_all o3_all co_all ch4_all ang_all'])
