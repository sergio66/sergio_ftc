...
for ii = 1:nprof;
   ...
   % Do top layer
   planck = bt2rad( fchan, prof.ptemp(1,ii) ) * ones(1,nang);
   ia = nang:-1:1;
   ...
   for jj = 2:lsurf
      planck = bt2rad( fchan, prof.ptemp(jj,ii) ) * ones(1,nang);
      inda = ind;
      ind = inda - nang;
   ...
   planck = bt2rad( fchan, reg_all_tsurf(ii) ) * ones(1, nang);
...
nang = nangrad;
secang = secangrad;
eval(['save old_radall_' typeband ' radall radtransall fchan ichan typeband nang nprof nchan bttransall btall temp_all h2o_all o3_all co_all ch4_all ang_all'])
